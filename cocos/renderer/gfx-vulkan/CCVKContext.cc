...
void APIENTRY CCVKEGLDebugProc(GLenum source, GLenum type, GLuint id, GLenum severity, GLsizei length, const GLchar* message, const void* userParam)
{
  String source_desc;
  switch (source)
  {
...
bool CCVKContext::MakeCurrent() {
  if (MakeCurrentImpl()) {
    if (!_isInitialized) {
      ...
      glDebugMessageCallbackKHR(CCVKEGLDebugProc, NULL);
#endif
