...
const GLenum CCVK_BLEND_FACTORS[] = {
  GL_ZERO,
  GL_ONE,
  GL_SRC_ALPHA,
  GL_DST_ALPHA,
      ...
      if (gpuBuffer->usage & GFXBufferUsageBit::VERTEX) {
        ...
        for(auto i = 0; i < GFX_MAX_BUFFER_BINDINGS; i++) {
          if(ubo[i] == gpuBuffer->glBuffer) {
            ...
            device->stateCache->glUniformBuffer = 0; ubo[i] = 0;
          }
        }
        if (device->stateCache->glUniformBuffer == gpuBuffer->glBuffer) {
          glBindBuffer(GL_UNIFORM_BUFFER, 0);
...
void CCVKCmdFuncCreateSampler(CCVKDevice* device, CCVKGPUSampler* gpuSampler) {
  ...
  gpuSampler->glWrapS = CCVK_WRAPS[(int)gpuSampler->addressU];
  gpuSampler->glWrapT = CCVK_WRAPS[(int)gpuSampler->addressV];
  gpuSampler->glWrapR = CCVK_WRAPS[(int)gpuSampler->addressW];
...
void CCVKCmdFuncCreateShader(CCVKDevice* device, CCVKGPUShader* gpuShader) {
  ...
  for (size_t i = 0; i < gpuShader->gpuStages.size(); ++i) {
    CCVKGPUShaderStage& gpu_stage = gpuShader->gpuStages[i];
  ...
  for (GLint i = 0; i < attr_count; ++i) {
    CCVKGPUInput& gpuInput = gpuShader->glInputs[i];
  ...
  if (block_count) {
    ...
    for (GLint i = 0; i < block_count; ++i) {
      CCVKGPUUniformBlock& gpuBlock = gpuShader->glBlocks[i];
      memset(gl_name, 0, sizeof(gl_name));
      ...
      if (gpuBlock.binding != GFX_INVALID_BINDING) {
        ...
        for (GLint u = 0; u < gl_block_uniforms; ++u) {
          CCVKGPUUniform& uniform = gpuBlock.glUniforms[u];
  ...
  if (gpuShader->samplers.size()) {
    ...
    for (size_t i = 0; i < gpuShader->glSamplers.size(); ++i) {
      ...
      CCVKGPUUniformSampler& gpuSampler = gpuShader->glSamplers[i];
  ...
  CCVKGPUUniformSamplerList active_gpu_samplers;
  ...
  for (GLint i = 0; i < glActiveUniforms; ++i) {
    ...
    if (is_sampler) {
      ...
      for (size_t s = 0; s < gpuShader->glSamplers.size(); ++s) {
        CCVKGPUUniformSampler& gpuSampler = gpuShader->glSamplers[s];
  ...
  if (active_gpu_samplers.size()) {
    ...
    for (size_t i = 0; i < active_gpu_samplers.size(); ++i) {
      CCVKGPUUniformSampler& gpuSampler = active_gpu_samplers[i];
...
void CCVKCmdFuncCreateInputAssembler(CCVKDevice* device, CCVKGPUInputAssembler* gpuInputAssembler) {
  ...
  for (size_t i = 0; i < gpuInputAssembler->glAttribs.size(); ++i) {
    CCVKGPUAttribute& gpuAttribute = gpuInputAssembler->glAttribs[i];
    const GFXAttribute& attrib = gpuInputAssembler->attributes[i];
    
    CCVKGPUBuffer* gpu_vb = (CCVKGPUBuffer*)gpuInputAssembler->gpuVertexBuffers[attrib.stream];
...
void CCVKCmdFuncCreateFramebuffer(CCVKDevice* device, CCVKGPUFramebuffer* gpuFBO) {
  if (gpuFBO->isOffscreen) {
    ...
    for (size_t i = 0; i < gpuFBO->gpuColorViews.size(); ++i) {
      CCVKGPUTextureView* gpuColorView = gpuFBO->gpuColorViews[i];
    ...
    if (gpuFBO->gpuDepthStencilView) {
      CCVKGPUTextureView* gpuDepthStencilView = gpuFBO->gpuDepthStencilView;
...
void CCVKCmdFuncExecuteCmds(CCVKDevice* device, CCVKCmdPackage* cmd_package) {
  ...
  CCVKStateCache* cache = device->stateCache;
  ...
  CCVKCmdBeginRenderPass* cmdBeginRenderPass = nullptr;
  ...
  for (uint i = 0; i < cmd_package->cmds.size(); ++i) {
    ...
    switch (cmd_type) {
      case GFXCmdType::BEGIN_RENDER_PASS: {
        CCVKCmdBeginRenderPass* cmd = cmd_package->beginRenderPassCmds[cmd_idx];
      ...
      case GFXCmdType::END_RENDER_PASS: {
        CCVKCmdBeginRenderPass* cmd = cmdBeginRenderPass;
        uint num_attachments = 0;
        for (uint j = 0; j < cmd->num_clear_colors; ++j) {
          const GFXColorAttachment& colorAttachment = gpuRenderPass->colorAttachments[j];
          if (colorAttachment.format != GFXFormat::UNKNOWN) {
      ...
      case GFXCmdType::BIND_STATES: {
        CCVKCmdBindStates* cmd = cmd_package->bindStatesCmds[cmd_idx];
        ...
        if (cache->dss.depthFunc != gpuPipelineState->dss.depthFunc) {
          glDepthFunc(CCVK_CMP_FUNCS[(int)gpuPipelineState->dss.depthFunc]);
        ...
        if (cache->dss.stencilFuncFront != gpuPipelineState->dss.stencilFuncFront ||
            ...
            glStencilFuncSeparate(GL_FRONT,
                                CCVK_CMP_FUNCS[(int)gpuPipelineState->dss.stencilFuncFront],
        ...
        if (cache->dss.stencilFailOpFront != gpuPipelineState->dss.stencilFailOpFront ||
            ...
            glStencilOpSeparate(GL_FRONT,
                              CCVK_STENCIL_OPS[(int)gpuPipelineState->dss.stencilFailOpFront],
                              CCVK_STENCIL_OPS[(int)gpuPipelineState->dss.stencilZFailOpFront],
                              CCVK_STENCIL_OPS[(int)gpuPipelineState->dss.stencilPassOpFront]);
        ...
        if (cache->dss.stencilFuncBack != gpuPipelineState->dss.stencilFuncBack ||
            ...
            glStencilFuncSeparate(GL_BACK,
                                CCVK_CMP_FUNCS[(int)gpuPipelineState->dss.stencilFuncBack],
        ...
        if (cache->dss.stencilFailOpBack != gpuPipelineState->dss.stencilFailOpBack ||
            ...
            glStencilOpSeparate(GL_BACK,
                              CCVK_STENCIL_OPS[(int)gpuPipelineState->dss.stencilFailOpBack],
                              CCVK_STENCIL_OPS[(int)gpuPipelineState->dss.stencilZFailOpBack],
                              CCVK_STENCIL_OPS[(int)gpuPipelineState->dss.stencilPassOpBack]);
        ...
        if (cache_target.blendEq != target.blendEq ||
          ...
          glBlendEquationSeparate(CCVK_BLEND_OPS[(int)target.blendEq],
                                  CCVK_BLEND_OPS[(int)target.blendAlphaEq]);
        ...
        if (cache_target.blendSrc != target.blendSrc ||
          ...
          glBlendFuncSeparate(CCVK_BLEND_FACTORS[(int)target.blendSrc],
                              CCVK_BLEND_FACTORS[(int)target.blendDst],
                              CCVK_BLEND_FACTORS[(int)target.blendSrcAlpha],
                              CCVK_BLEND_FACTORS[(int)target.blendDstAlpha]);
        ...
        if (cmd->gpuBindingLayout && gpuPipelineState->gpuShader) {
          for(size_t j = 0; j < cmd->gpuBindingLayout->gpuBindings.size(); ++j) {
            const CCVKGPUBinding& gpuBinding = cmd->gpuBindingLayout->gpuBindings[j];
              ...
              case GFXBindingType::UNIFORM_BUFFER: {
                if (gpuBinding.gpuBuffer) {
                  for (size_t k = 0; k < gpuPipelineState->gpuShader->glBlocks.size(); ++k) {
                    const CCVKGPUUniformBlock& gpuBlock = gpuPipelineState->gpuShader->glBlocks[k];
              ...
              case GFXBindingType::SAMPLER: {
                if (gpuBinding.gpuSampler) {
                  for (size_t k = 0; k < gpuPipelineState->gpuShader->glSamplers.size(); ++k) {
                    const CCVKGPUUniformSampler& gpuSampler = gpuPipelineState->gpuShader->glSamplers[k];
          ...
          if (device->useVAO()) {
            ...
            if (!glVAO) {
              ...
              for (size_t j = 0; j < gpuPipelineState->gpuShader->glInputs.size(); ++j) {
                const CCVKGPUInput& gpuInput = gpuPipelineState->gpuShader->glInputs[j];
                for (size_t a = 0; a < gpuInputAssembler->attributes.size(); ++a) {
                  const CCVKGPUAttribute& gpuAttribute = gpuInputAssembler->glAttribs[a];
      ...
      case GFXCmdType::DRAW: {
        CCVKCmdDraw* cmd = cmd_package->drawCmds[cmd_idx];
      ...
      case GFXCmdType::UPDATE_BUFFER: {
        CCVKCmdUpdateBuffer* cmd = cmd_package->updateBufferCmds[cmd_idx];
        CCVKCmdFuncUpdateBuffer(device, cmd->gpuBuffer, cmd->buffer, cmd->offset, cmd->size);
        break;
      }
      case GFXCmdType::COPY_BUFFER_TO_TEXTURE: {
        CCVKCmdCopyBufferToTexture* cmd = cmd_package->copyBufferToTextureCmds[cmd_idx];
        CCVKCmdFuncCopyBuffersToTexture(device, &(cmd->gpuBuffer->buffer), cmd->gpuTexture, cmd->regions);
        break;
      }
      default:
        break;
