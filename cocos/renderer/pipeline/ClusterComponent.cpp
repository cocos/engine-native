...
void ClusterComponent::update(scene::Camera* camera) {
    if (!_initialized) return;

    scene::Plane near     = camera->frustum.planes[4];
    scene::Plane far      = camera->frustum.planes[5];
    Vec3         position = camera->position;
    float        nearClip = near.n.dot(position) / near.n.length() + near.d;
    float        farClip  = far.n.dot(position) / far.n.length() + far.d;

    _constants[0] = 1.0f;                                     // near far not correct, hard code here
    _constants[1] = 1000.0f;                                  // near far not correct, hard code here
    _constants[4] = 0.0f;                                     // no viewport instead of width height
    _constants[5] = 0.0f;                                     // no viewport instead of width height
    _constants[6] = static_cast<float>(_device->getWidth());  // no viewport instead of width height
    _constants[7] = static_cast<float>(_device->getHeight()); // no viewport instead of width height
    _constants[8] = static_cast<float>(_device->getWidth());
    _constants[9] = static_cast<float>(_device->getHeight());
    memcpy(_constants.data() + 12, camera->matView.m, sizeof(cc::Mat4));
    memcpy(_constants.data() + 28, camera->matProjInv.m, sizeof(cc::Mat4));

    _constantsBuffer->update(_constants.data(), 3 * sizeof(Vec4) + 2 * sizeof(Mat4));
    updateLights(camera);
...
void ClusterComponent::initBuildingSatge() {
    ...
    sources.glsl4 = StringUtil::format(
		...
		void main() {
			uint clusterIndex = gl_GlobalInvocationID.z * uvec3(16, 8, 4).x * uvec3(16, 8, 4).y +
								gl_GlobalInvocationID.y * uvec3(16, 8, 4).x +
    ...
    shaderInfo.blocks = {
        {0, 0, "CCConst", {{"cc_nearFar", gfx::Type::FLOAT4, 1}, {"cc_viewPort", gfx::Type::FLOAT4, 1}, {"cc_screenSize", gfx::Type::FLOAT4, 1}, {"cc_matView", gfx::Type::MAT4, 1}, {"cc_matProjInv", gfx::Type::MAT4, 1}}, 1},
    };
