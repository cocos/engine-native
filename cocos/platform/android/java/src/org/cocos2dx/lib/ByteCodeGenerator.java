...
public class ByteCodeGenerator {
    ...
    public static <T, G extends T> Object newInstance(String superClassName, String[] interfaces) {
        ...
        TypeId<T> superClass = TypeId.get("L" + bc.superClass + ";");
        TypeId<G> generatedType = TypeId.get("L" + bc.getClassName() + ";");

        bc.implConstructor(maker, superClass, generatedType);

        List<MethodRecord> virtualMethods = bc.findAllVirtualMethod();
        for (MethodRecord m : virtualMethods) {
            bc.implVirtualFunction(maker, m, superClass, generatedType);
        }
    ...
    private <T, G extends T> void implConstructor(DexMaker maker, TypeId<T> superClass, TypeId<G> generatedType){

        TypeId<?>[] interfaceTypes = new TypeId<?>[interfaces.size()];
        for (int i = 0; i < interfaces.size(); i++) {
            String interfaceName = interfaces.get(i);
            interfaceTypes[i] = TypeId.get("L" + interfaceName + ";");
        }
        maker.declare(generatedType, getClassNameWithDots() + ".generated", Modifier.PUBLIC, superClass, interfaceTypes);
    ...
    private <T, G extends T> void implVirtualFunction(DexMaker maker, MethodRecord m, TypeId<T> superClass, TypeId<G> generatedType){
