...
static bool js_scene_Shadow_get_saturation(se::State& s) // NOLINT(readability-identifier-naming, google-runtime-references)
{
    auto* cobj = SE_THIS_OBJECT<cc::scene::Shadow>(s);
...
static bool js_scene_Shadow_set_saturation(se::State& s) // NOLINT(readability-identifier-naming, google-runtime-references)
{
    const auto& args = s.args();
    auto* cobj = SE_THIS_OBJECT<cc::scene::Shadow>(s);
...
static bool js_scene_Shadow_constructor(se::State& s) // NOLINT(readability-identifier-naming, google-runtime-references)
{
    ...
    if (argc > 13 && !args[13].isUndefined()) {
        ok &= sevalue_to_native(args[13], &(cobj->orthoSize), nullptr);
    }
    if (argc > 14 && !args[14].isUndefined()) {
        ok &= sevalue_to_native(args[14], &(cobj->saturation), nullptr);
    }
